...
public abstract class AbstractPgJsonbController {
    ...
    public Result<PgProductProfile> getById(@PathVariable Long id) {
        return Result.ok(service.getById(id));
    }

    /** 查询全部商品扩展记录并按主键升序返回。 */
    @GetMapping("/product-profiles")
    public Result<List<PgProductProfile>> list() {
        return Result.ok(service.list());
    }

    /** 动态演示 ->>、@>、键存在和 #>> 四类 JSONB 查询。 */
    @PostMapping("/product-profiles/search")
    public Result<List<PgProductProfile>> search(
    ...
    public Result<PgProductProfile> mergeAttributes(
    ...
    public Result<PgProductProfile> updateWarrantyMonths(
            @PathVariable Long id,
    ...
    public Result<PgProductProfile> deleteAttribute(
            @PathVariable Long id,
