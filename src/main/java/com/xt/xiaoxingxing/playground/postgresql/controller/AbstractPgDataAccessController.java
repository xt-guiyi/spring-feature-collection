...
import com.xt.xiaoxingxing.playground.postgresql.service.PgDataAccessService;
...
public abstract class AbstractPgDataAccessController {
    ...
    @PostMapping("/users")
    public Result<Long> createUser(@RequestBody PgUser user) {
        return Result.ok(service.createUser(user));
    }

    /** 按主键查询用户；记录不存在时两套实现都抛出相同业务异常。 */
    @GetMapping("/users/{id}")
    public Result<PgUser> getUser(@PathVariable Long id) {
        return Result.ok(service.getUser(id));
    }

    /** 查询全部用户并按 ID 升序返回。 */
    @GetMapping("/users")
    public Result<List<PgUser>> listUsers() {
        return Result.ok(service.listUsers());
    }

    /** 按路径 ID 更新用户，Body 中只需提供需要修改的非空字段。 */
    @PutMapping("/users/{id}")
    public Result<Boolean> updateUser(@PathVariable Long id, @RequestBody PgUser user) {
        user.setId(id);
    ...
    public Result<Long> createIdCard(@RequestBody PgIdCard idCard) {
        return Result.ok(service.createIdCard(idCard));
    }

    /** 按身份证记录主键查询；这里的主键不是 card_number。 */
    @GetMapping("/id-cards/{id}")
    public Result<PgIdCard> getIdCard(@PathVariable Long id) {
        return Result.ok(service.getIdCard(id));
    }

    /** 查询全部身份证记录，便于与 LEFT JOIN 的“用户保留侧”结果进行对照。 */
    @GetMapping("/id-cards")
    public Result<List<PgIdCard>> listIdCards() {
        return Result.ok(service.listIdCards());
    }

    /** 局部更新身份证记录；未传字段不会被 XML set 或 updateById 修改。 */
    @PutMapping("/id-cards/{id}")
    public Result<Boolean> updateIdCard(@PathVariable Long id, @RequestBody PgIdCard idCard) {
        idCard.setId(id);
        return Result.ok(service.updateIdCard(idCard));
    }
    ...
    @PostMapping("/orders")
    public Result<Long> createOrder(@RequestBody PgOrder order) {
        return Result.ok(service.createOrder(order));
    }

    /** 按订单主键查询订单头，订单商品明细需要通过关联表接口或 JOIN 案例查询。 */
    @GetMapping("/orders/{id}")
    public Result<PgOrder> getOrder(@PathVariable Long id) {
        return Result.ok(service.getOrder(id));
    }

    /** 查询全部订单头并保持稳定排序，方便比较两套实现返回值。 */
    @GetMapping("/orders")
    public Result<List<PgOrder>> listOrders() {
        return Result.ok(service.listOrders());
    }

    /** 局部更新订单；金额若提供必须大于等于零。 */
    @PutMapping("/orders/{id}")
    public Result<Boolean> updateOrder(@PathVariable Long id, @RequestBody PgOrder order) {
        order.setId(id);
        return Result.ok(service.updateOrder(order));
    }
    ...
    public Result<Long> createProduct(@RequestBody PgProduct product) {
        return Result.ok(service.createProduct(product));
    }

    /** 按商品主键查询当前价格与库存。 */
    @GetMapping("/products/{id}")
    public Result<PgProduct> getProduct(@PathVariable Long id) {
        return Result.ok(service.getProduct(id));
    }

    /** 查询全部商品；可与 FULL OUTER JOIN 中的未售商品结果对照学习。 */
    @GetMapping("/products")
    public Result<List<PgProduct>> listProducts() {
        return Result.ok(service.listProducts());
    }

    /** 局部更新商品，允许单独改名、改价或调整库存，但不允许负值。 */
    @PutMapping("/products/{id}")
    public Result<Boolean> updateProduct(@PathVariable Long id, @RequestBody PgProduct product) {
        product.setId(id);
        return Result.ok(service.updateProduct(product));
    }
    ...
    public Result<Long> createOrderProduct(@RequestBody PgOrderProduct orderProduct) {
        return Result.ok(service.createOrderProduct(orderProduct));
    }

    /** 按关联表自身主键查询一条订单商品明细。 */
    @GetMapping("/order-products/{id}")
    public Result<PgOrderProduct> getOrderProduct(@PathVariable Long id) {
        return Result.ok(service.getOrderProduct(id));
    }

    /** 查询全部订单商品明细；它是订单和商品多对多关系的中间表。 */
    @GetMapping("/order-products")
    public Result<List<PgOrderProduct>> listOrderProducts() {
        return Result.ok(service.listOrderProducts());
    }
    ...
    public Result<Boolean> updateOrderProduct(@PathVariable Long id,
                                              @RequestBody PgOrderProduct orderProduct) {
        orderProduct.setId(id);
        return Result.ok(service.updateOrderProduct(orderProduct));
    }
    ...
    public Result<List<PgUser>> searchUsers(@RequestBody(required = false) UserQueryRequest request) {
        return Result.ok(service.searchUsers(request));
    }

    /** 对比 MyBatis LIMIT/OFFSET 与 MyBatis-Plus 分页插件。 */
    @GetMapping("/users/page")
    public Result<PageResult<PgUser>> pageUsers(@RequestParam(defaultValue = "1") int pageNum,
                                                @RequestParam(defaultValue = "10") int pageSize) {
        return Result.ok(service.pageUsers(pageNum, pageSize));
    }

    /** 对比 XML foreach 多 VALUES 与事务内多次 BaseMapper insert。 */
    @PostMapping("/users/batch")
    public Result<Boolean> batchCreateUsers(@RequestBody List<PgUser> users) {
        return Result.ok(service.batchCreateUsers(users));
    }
    ...
    public Result<List<PgOrder>> searchOrders(@RequestBody(required = false) OrderQueryRequest request) {
        return Result.ok(service.searchOrders(request));
    }

    /** 对比手写 COUNT + LIMIT/OFFSET 与 MyBatis-Plus Page 分页插件。 */
    @GetMapping("/orders/page")
    public Result<PageResult<PgOrder>> pageOrders(@RequestParam(defaultValue = "1") int pageNum,
                                                  @RequestParam(defaultValue = "10") int pageSize) {
        return Result.ok(service.pageOrders(pageNum, pageSize));
    }
    ...
    public Result<List<PgProduct>> searchProducts(@RequestBody(required = false) ProductQueryRequest request) {
        return Result.ok(service.searchProducts(request));
    }
    ...
    public Result<List<OrderDetailVO>> innerOrderDetails(@RequestParam String orderNo) {
        return Result.ok(service.getInnerOrderDetails(orderNo));
    }

    /** LEFT JOIN：保留所有用户，未认证用户的身份证字段为空。 */
    @GetMapping("/joins/left/users-id-cards")
    public Result<List<UserWithIdCardVO>> leftUsersWithIdCard() {
        return Result.ok(service.getLeftUsersWithIdCard());
    }

    /** RIGHT JOIN：保留右侧所有用户，没有订单时订单字段为空。 */
    @GetMapping("/joins/right/users-orders")
    public Result<List<UserOrderVO>> rightUsersWithOrders() {
        return Result.ok(service.getRightUsersWithOrders());
    }

    /** FULL OUTER JOIN：审计未售商品和引用不到商品的订单明细。 */
    @GetMapping("/joins/full/products-order-items")
    public Result<List<ProductOrderAuditVO>> fullProductOrderAudit() {
        return Result.ok(service.getFullProductOrderAudit());
    }

    /** CROSS JOIN：生成受 limit 保护的用户商品笛卡尔积。 */
    @GetMapping("/joins/cross/user-products")
    public Result<List<UserProductCandidateVO>> crossUserProductCandidates(
            @RequestParam(defaultValue = "20") int limit) {
        return Result.ok(service.getCrossUserProductCandidates(limit));
    }

    /** PostgreSQL LATERAL JOIN：查询每个用户最新一笔订单。 */
    @GetMapping("/joins/lateral/latest-orders")
    public Result<List<UserOrderVO>> latestOrderPerUser() {
        return Result.ok(service.getLatestOrderPerUser());
    }
    ...
    public Result<List<UserOrderStatVO>> userOrderStats() {
        return Result.ok(service.getUserOrderStats());
    }

    /** 聚合子查询与 LIMIT：查询消费金额前 N 的用户。 */
    @GetMapping("/queries/top-spending-users")
    public Result<List<UserOrderStatVO>> topSpendingUsers(@RequestParam(defaultValue = "10") int limit) {
        return Result.ok(service.getTopSpendingUsers(limit));
    }

    /** EXISTS：查询至少拥有一笔订单的用户。 */
    @GetMapping("/queries/users-with-orders")
    public Result<List<UserSimpleVO>> usersWithOrders() {
        return Result.ok(service.getUsersWithOrders());
    }

    /** NOT EXISTS：查询完全没有订单的用户。 */
    @GetMapping("/queries/users-without-orders")
    public Result<List<UserSimpleVO>> usersWithoutOrders() {
        return Result.ok(service.getUsersWithoutOrders());
    }

    /** HAVING：在聚合后按订单数量过滤用户。 */
    @GetMapping("/queries/users-by-order-count")
    public Result<List<UserOrderStatVO>> usersByOrderCount(
            @RequestParam(defaultValue = "1") int minOrderCount) {
        return Result.ok(service.getUsersByOrderCount(minOrderCount));
    }

    /** UNION ALL：拼接互斥的活跃用户与非活跃用户查询结果。 */
    @GetMapping("/queries/users-union-all")
    public Result<List<UserSimpleVO>> usersUnionAll() {
        return Result.ok(service.getUsersUnionAll());
    }

    /** CASE WHEN：把订单状态码映射为中文名称。 */
    @GetMapping("/queries/order-status-names")
    public Result<List<OrderStatusVO>> orderStatusWithName() {
        return Result.ok(service.getOrderStatusWithName());
    }
    ...
    public Result<List<OrderStatusVO>> orderStatusWithEnumMapping() {
        return Result.ok(service.getOrderStatusWithEnumMapping());
    }

    /** COALESCE：手机号为空时返回“未填写”。 */
    @GetMapping("/queries/user-contacts")
    public Result<List<UserContactVO>> usersWithCoalescePhone() {
        return Result.ok(service.getUsersWithCoalescePhone());
    }

    /** PostgreSQL 日期转换与年份提取。 */
    @GetMapping("/queries/user-date-stats")
    public Result<List<UserDateStatVO>> userDateStats() {
        return Result.ok(service.getUserDateStats());
    }

    /** ROW_NUMBER 窗口函数：生成用户消费连续排名。 */
    @GetMapping("/queries/user-spending-rank")
    public Result<List<UserRankVO>> userSpendingRank() {
        return Result.ok(service.getUserSpendingRank());
    }

    /** CTE：先统计用户消费，再划分 HIGH、MEDIUM、LOW 等级。 */
    @GetMapping("/queries/user-spending-levels")
    public Result<List<UserSpendingLevelVO>> userSpendingLevels() {
        return Result.ok(service.getUserSpendingLevels());
    }

    /** 商品维度 GROUP BY：统计销量、销售额并同时返回当前库存。 */
    @GetMapping("/queries/product-sales-stats")
    public Result<List<ProductSalesStatVO>> productSalesStats() {
        return Result.ok(service.getProductSalesStats());
    }
    ...
    public Result<CompleteOrderResponse> createCompleteOrder(
        ...
        return Result.ok(service.createCompleteOrder(request));
    }
}
