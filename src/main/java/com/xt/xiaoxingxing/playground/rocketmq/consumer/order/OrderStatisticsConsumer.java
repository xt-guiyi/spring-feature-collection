...
@RocketMQMessageListener(endpoints = "${playground.rocketmq.endpoints}",
        topic = RocketMqNames.NORMAL_TOPIC, consumerGroup = RocketMqNames.ORDER_STATISTICS_GROUP,
...
public class OrderStatisticsConsumer implements RocketMQListener {
    ...
    public ConsumeResult consume(MessageView messageView) {
        return consumerSupport.handle(
                messageView, RocketMqNames.ORDER_STATISTICS_GROUP,
                RocketConsumerSupport.ORDER_EVENT_ROUTE_CONTRACT, consumerService::handleStatistics);
    }
}
