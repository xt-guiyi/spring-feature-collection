...
import com.xt.xiaoxingxing.playground.flowable.service.FlowableTaskService;
...
public class FlowableTaskController {
    ...
    /** 分页查询当前活动任务。 */
    @GetMapping("/tasks")
    public Result<PageResult<FlowableTaskVO>> tasks(@Valid @ModelAttribute TaskQueryRequest request) {
        return Result.ok(taskService.tasks(request));
    }

    /** 候选人领取任务。 */
    @PostMapping("/tasks/{taskId}/claim")
    public Result<FlowableTaskVO> claim(
    ...
    public Result<LeaveRequestVO> complete(
            @PathVariable String taskId,
