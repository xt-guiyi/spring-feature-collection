package com.xt.xiaoxingxing.playground.features.rocketmq.service;

import com.xt.xiaoxingxing.playground.features.rocketmq.constants.OrderMqConstants;
import com.xt.xiaoxingxing.playground.features.rocketmq.dto.request.CreateOrderItemRequest;
...
import com.xt.xiaoxingxing.playground.features.rocketmq.enums.OrderStatus;
import com.xt.xiaoxingxing.playground.features.rocketmq.mapper.OrderMapper;
import com.xt.xiaoxingxing.playground.features.rocketmq.repository.TransactionRecordRepository;
import com.xt.xiaoxingxing.playground.features.rocketmq.util.RocketMqUtil;
...
public class OrderService {
    ...
    private OrderResponse resolveLocalFailure(String transactionId,
        ...
        if (current != null && current.getStatus() == MqTransactionStatus.COMMITTED) {
            ...
            OrderResponse persisted = toResponse(order, orderMapper.selectOrderItems(order.getId()));
            commitHalfOrWarn(half, transactionId, operation);
            return persisted;
        }
    ...
    private OrderResponse payOrderLocally(Long orderId) {
        ...
        return toResponse(order, orderMapper.selectOrderItems(orderId));
    }
    ...
    private OrderResponse toResponse(Order source, List<OrderItem> items) {
        int itemCount = items.stream().mapToInt(OrderItem::getQuantity).sum();
        return new OrderResponse(source.getId(), source.getOrderNo(), source.getStatus(),
                source.getTotalAmount(), itemCount);
    }
