...
public abstract class AbstractArticleCrudController {
    ...
    public Result<ArticleDetailResponse> create(@Valid @RequestBody ArticleWriteRequest request) {
        return Result.ok(service.create(request));
    }

    /** 按 ID 查询文章检索投影。 */
    @GetMapping("/articles/{id}")
    public Result<ArticleDetailResponse> getById(@PathVariable String id) {
        return Result.ok(service.getById(id));
    }

    /** 更新指定文章检索投影。 */
    @PutMapping("/articles/{id}")
    public Result<ArticleDetailResponse> update(@PathVariable String id,
