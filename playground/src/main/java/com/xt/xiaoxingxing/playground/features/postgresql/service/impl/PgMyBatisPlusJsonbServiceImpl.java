...
import com.xt.xiaoxingxing.playground.features.postgresql.mapper.PgProductProfilePlusMapper;
...
public class PgMyBatisPlusJsonbServiceImpl implements PgJsonbService {
    ...
    public PgProductProfileResponse getById(Long id) {
        ...
        return toResponse(BusinessAssert.notNull(profileMapper.selectById(id), "商品扩展信息不存在"));
    }

    @Override
    public List<PgProductProfileResponse> list() {
        return profileMapper.selectList(Wrappers.<PgProductProfile>lambdaQuery()
                .orderByAsc(PgProductProfile::getId)).stream().map(this::toResponse).toList();
    }
    ...
    public List<PgProductProfileResponse> search(ProductProfileSearchRequest request) {
        ...
        return profileMapper.selectList(wrapper).stream().map(this::toResponse).toList();
    }
