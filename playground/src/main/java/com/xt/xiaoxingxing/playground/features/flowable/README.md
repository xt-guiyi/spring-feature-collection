...
服务端仍会逐个通过 Playground 自己的 `demo.users` 检查这些用户是否存在且状态为 `ACTIVE`；因此 1、2 天和 3、4、5 天申请通常可以直接演示，6 天以上申请需要先在 `demo.users` 准备 ID 为 `3` 的 ACTIVE 用户（或者修改 `FlowableApprovalUserResolver` 中的学习映射）。
...
`docker/init/postgres/demo/flowable/schema/schema.sql` 只保存 Flowable 8.0.0 官方 PostgreSQL 建表脚本，`docker/init/postgres/demo/flowable/data/data.sql` 保存 Flowable 官方初始化数据，`docker/init/postgres/demo/schema/flowable-leave-business-schema.sql` 只保存本项目请假业务表。PostgreSQL 数据库和初始化脚本由人工按目标数据库执行；应用配置固定 `flowable.database-schema-update=false`，应用启动不会替代迁移脚本。
