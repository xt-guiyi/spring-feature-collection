...
import com.xt.xiaoxingxing.playground.features.elasticsearch.service.ArticleIndexManager;
...
public class ArticleIndexController {
    ...
    public Result<ArticleIndexStatusResponse> initialize() {
        return Result.ok(articleIndexManager.initialize());
    }

    /** 写入固定的演示文章数据。 */
    @PostMapping("/seed")
    public Result<ArticleBulkResultResponse> seed() {
        return Result.ok(articleIndexManager.seed());
    }

    /** 查询当前文章索引和别名状态。 */
    @GetMapping("/status")
    public Result<ArticleIndexStatusResponse> status() {
        return Result.ok(articleIndexManager.status());
    }

    /** 重建文章索引并切换查询别名。 */
    @PostMapping("/rebuild")
    public Result<ArticleRebuildResponse> rebuild() {
