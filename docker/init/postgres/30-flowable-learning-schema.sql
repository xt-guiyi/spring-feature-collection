-- Flowable 8.0.0 PostgreSQL 初始化脚本（官方 tag flowable-8.0.0，commit 0779d68e5a3385b74d8acb8bc37901ff54513249）。
-- 官方脚本按 common -> engine -> history -> dmn 顺序合并；仅由 PostgreSQL 空数据卷首次初始化时执行。
-- 应用配置 flowable.database-schema-update=false，启动阶段只校验 Flowable schema 版本，不在业务连接上自动建表。
-- 来源文件：flowable.postgres.create.common.sql、flowable.postgres.create.engine.sql、
-- flowable.postgres.create.history.sql、flowable.postgres.create.dmn.sql。
...
-- 本项目流程引擎学习模块的业务表（不与 Flowable ACT_* 系统表建立外键，便于独立演示）。
